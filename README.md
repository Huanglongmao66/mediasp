...
- 🔍 **全局搜索**: 跨视频/音乐/图片/小说/电台的全站搜索，分类展示结果
- 💾 **播放记录**: 自动保存播放进度，下次打开继续播放
- ❤️ **收藏管理**: 分类收藏所有媒体类型，Tab切换快速访问
- 📂 **本地扫描**: 自动扫描设备中的视频文件
- 🌙 **暗黑模式**: 支持亮色/暗色主题切换
...
- ✅ 主题动态应用（设置页修改即时生效）
- ✅ HomeScreen 底部导航栏优化
- ✅ 全平台编译测试通过（Desktop + JS目标）
- ✅ 扩展模块框架（音乐/图片/小说/电台）
- ✅ 扩展模块详情页（音乐播放/图片查看/小说阅读/电台播放）
- ✅ JS平台编译修复（ObservableSettings、TimeFormatter、平台API兼容）
- ✅ 全局搜索功能（视频/音乐/图片/小说/电台分类搜索）
- ✅ 收藏页面分类展示（Tab切换，各媒体类型独立收藏）
- ✅ Android平台MainActivity Settings类型修复
- ⏳ 单元测试待补充
